...
(async () => {
  ...
  let infoList = [`Use：${bytesToSize(used)} | ${bytesToSize(total)}`];

  if (resetDayLeft) {
    infoList.push(`Reset：The Remaining${resetDayLeft}Day`);
  }
  if (expire) {
    if (/^[\d.]+$/.test(expire)) expire *= 1000;
    infoList.push(`Expiration：${formatTime(expire)}`);
  }
...
function getUserInfo(url) {
  ...
  return new Promise((resolve, reject) =>
    $httpClient.head(request, (err, resp) => {
      ...
      reject("Connection does not provide traffic information");
    })
  );
}
...
function sendNotification(usageRate, expire, infoList) {
  ...
  if (usageRate > count.usageRate && resetDay != today) {
    $notification.post(
      `${title} | Lack Of Surplus Flow${Math.ceil(100 - usageRate)}%`,
      subtitle,
      body
    );
  ...
  if (resetDayLeft && resetDayLeft < count.resetDayLeft && resetDay != today) {
    $notification.post(
      `${title} | Traffic Will Be In${resetDayLeft}Reset After Dat`,
      subtitle,
      body
    );
  ...
  if (resetDay == today && count.resetDay && usageRate < 5) {
    $notification.post(`${title} | Traffic Has Been Reset`, subtitle, body);
    count.resetDay = 0;
  }
  if (expire) {
    ...
    if (diff < count.expire) {
      $notification.post(
        `${title} | The Rest Of The Package Is Insufficient${Math.ceil(diff)}Day`,
        subtitle,
        body
      );
      count.expire -= 5;
